...
- **[2026-02-10]** 중복 차감 버그 수정(완전판): (1) 리부팅 감지: `REBOOT_DETECTION_THRESHOLD_MS = 5000L` 임계값 도입. `lastSync > now`만으로 판단하지 않고, 5초 초과 역행 또는 `now < lastSync/2` 휴리스틱 시 리부팅으로 간주; 리부팅 시 `lastSync = snapshotNow`로 설정(0이 아님). 미세 역행(5초 미만)은 스레드 경합으로 간주하여 해당 틱 스킵. (2) TOCTOU 방지: `syncState()`에서 `snapshotNow` 파라미터 제거, 락 내부에서 `timeSource.elapsedRealtime()` 호출. (3) 3중 방어선: 제1방어선 BOOT_COMPLETED 리시버(DailyResetReceiver에서 `setTimeCreditLastSyncTimeSync(0L)`), 제2방어선 syncState 내 런타임 임계값, 제3방어선 onStartCommand 맨 처음 실패 안전(lastSync > elapsedRealtime() 시 lastSync 0으로 초기화). (4) 능동 감시: 세션 모니터 루프에서 UsageStats 권한 있을 때만 `getCurrentForegroundPackageInternal()` 호출로 이탈만 반영(보수적 폴백); try-catch 및 [ActiveCorrection] 로그. (5) PreferenceManager에 `setTimeCreditLastSyncTimeSync()`(commit) 추가. (6) `calculateUsageSinceLastSync`, `performFinalDeductionOnSessionEnd`, `tryRecoverSessionIfKilled` 리부팅 감지도 임계값 기반으로 통일. 영향: TimeCreditBackgroundService, DailyResetReceiver, PreferenceManager, arch_events.
