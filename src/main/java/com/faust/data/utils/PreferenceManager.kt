...
class PreferenceManager(private val context: Context) {
    ...
    /** 마지막 크레딧 정산 시점 저장 (동기 저장, commit 사용). 부팅 직후 등 정합성이 중요한 경로에서 사용. */
    fun setTimeCreditLastSyncTimeSync(time: Long) {
        try {
            val success = prefs.edit().putLong(KEY_TIME_CREDIT_LAST_SYNC_TIME, time).commit()
            if (!success) {
                Log.e(TAG, "Failed to commit time credit last sync time")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to set time credit last sync time (sync)", e)
        }
    }
