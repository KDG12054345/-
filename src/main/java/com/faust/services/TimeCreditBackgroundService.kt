...
class TimeCreditBackgroundService : LifecycleService() {
    ...
    @VisibleForTesting
    internal var timeSource: TimeSource = SystemTimeSource()
    ...
    companion object {
        ...
        /** 리부팅 감지 임계값: 5초 이상 역행 시 리부팅으로 판단. 스레드 경합 역행은 ms 단위. */
        private const val REBOOT_DETECTION_THRESHOLD_MS = 5000L
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (lastSync > nowElapsed) {
            Log.w(TAG, "${AppLog.CONSISTENCY} Fail-safe: lastSync=$lastSync > uptime=$nowElapsed, resetting lastSync")
    ...
    private fun tryRecoverSessionIfKilled() {
        ...
        val lastSync = preferenceManager.getTimeCreditLastSyncTime()
        val nowElapsed = timeSource.elapsedRealtime()
        val lastAliveDiff = if (lastAlive > nowElapsed) lastAlive - nowElapsed else 0L
        val startDiff = if (startElapsed > nowElapsed) startElapsed - nowElapsed else 0L
        val lastSyncDiff = if (lastSync > nowElapsed) lastSync - nowElapsed else 0L
        if (lastAliveDiff > REBOOT_DETECTION_THRESHOLD_MS ||
            startDiff > REBOOT_DETECTION_THRESHOLD_MS ||
            lastSyncDiff > REBOOT_DETECTION_THRESHOLD_MS) {
            Log.w(TAG, "${AppLog.CONSISTENCY} Reboot detected in recovery (lastAlive diff=${lastAliveDiff}ms, start diff=${startDiff}ms, lastSync diff=${lastSyncDiff}ms)")
        ...
        sessionMonitorJob = serviceScope.launch {
            while (isActive) {
                try {
                    ...
                    if (isScreenOn && !isPaused && hasUsageStatsPermission()) {
                        try {
                            getCurrentForegroundPackageInternal()?.let { currentForeground ->
                                if (currentForeground != sessionPkg) {
                                    val prev = lastKnownForegroundPackage
                                    setLastKnownForegroundPackageInternal(currentForeground)
                                    Log.d(TAG, "${AppLog.CONSISTENCY} [ActiveCorrection] Event lost detected! Force updating pkg: $prev -> $currentForeground")
                                }
                            }
                        } catch (e: Exception) {
                            Log.w(TAG, "Active monitor failed", e)
                        }
                    }
                    val isDormant = when {
    ...
    private fun performFinalDeductionOnSessionEnd() {
        ...
        val lastSyncDiff = if (lastSync > snapshotNow) lastSync - snapshotNow else 0L
        val sessionStartDiff = if (sessionStart > snapshotNow) sessionStart - snapshotNow else 0L
        if (lastSyncDiff > REBOOT_DETECTION_THRESHOLD_MS || sessionStartDiff > REBOOT_DETECTION_THRESHOLD_MS) {
            Log.w(TAG, "${AppLog.CONSISTENCY} Reboot detected in final deduction (lastSync diff=${lastSyncDiff}ms, sessionStart diff=${sessionStartDiff}ms)")
    ...
    private fun calculateUsageSinceLastSync(snapshotNow: Long): Long {
        val lastSync = preferenceManager.getTimeCreditLastSyncTime()
        if (lastSync > snapshotNow && (lastSync - snapshotNow) > REBOOT_DETECTION_THRESHOLD_MS) {
        ...
        if (fromElapsed > snapshotNow && (fromElapsed - snapshotNow) > REBOOT_DETECTION_THRESHOLD_MS) {
     ...
     * 락 내부에서 timeSource.elapsedRealtime() 호출하여 TOCTOU 방지.
     *
     ...
     */
    private fun syncState(reason: String? = null, balanceOverride: Long? = null, synchronous: Boolean = false) {
        if (!preferenceManager.isCreditSessionActive()) return
        ...
        synchronized(syncStateLock) {
            ...
            val snapshotNowLocked = timeSource.elapsedRealtime()
            val timeDiff = lastSyncTime - snapshotNowLocked
            val isReboot = timeDiff > REBOOT_DETECTION_THRESHOLD_MS ||
                (timeDiff > 0L && snapshotNowLocked < lastSyncTime / 2)
            if (isReboot) {
                Log.w(TAG, "${AppLog.CONSISTENCY} Reboot detected (diff=${timeDiff}ms, now=${snapshotNowLocked}, lastSync=${lastSyncTime}), setting lastSync=now")
                preferenceManager.setTimeCreditLastSyncTime(snapshotNowLocked)
            } else if (timeDiff > 0L) {
                Log.w(TAG, "${AppLog.CONSISTENCY} Race condition detected (diff=${timeDiff}ms), skipping this tick")
                return@synchronized
            } else if (snapshotNowLocked == lastSyncTime) {
                idempotencySkipCount.incrementAndGet()
                Log.d(TAG, "${AppLog.CONSISTENCY} Idempotency guard - duplicate event (snapshot=$snapshotNowLocked == lastSync=$lastSyncTime)")
                return@synchronized
            }
    ...
    /**
     * PACKAGE_USAGE_STATS 권한이 있는지 확인.
     * AppOpsManager를 사용하여 런타임 권한 상태 확인.
     */
    private fun hasUsageStatsPermission(): Boolean {
        ...
        return try {
            ...
            if (Log.isLoggable(TAG, Log.DEBUG)) {
                Log.d(TAG, "UsageStats 권한 확인 실패: ${e.message}")
            }
            false
        }
    }
